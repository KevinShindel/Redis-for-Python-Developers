...
class SiteStatsDaoRedis(SiteStatsDaoBase, RedisDaoBase):
    ...
    def _update_optimized(self, key: str, meter_reading: MeterReading,
        ...
        if pipeline is None:
            ...
            execute = True
        setattr(pipeline, 'transaction', True)  # deal with transaction
        ...
        CompareAndUpdateScript(redis=self.redis).update_if_greater(pipeline=pipeline,
                                                                   key=key,
                                                                   field=SiteStats.MAX_WH,
                                                                   value=meter_reading.wh_generated)
        CompareAndUpdateScript(redis=self.redis).update_if_less(pipeline=pipeline,
                                                                key=key,
