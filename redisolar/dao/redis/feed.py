...
class FeedDaoRedis(FeedDaoBase, RedisDaoBase):
    ...
    def _insert(self, meter_reading: MeterReading, pipeline: redis.client.Pipeline) -> None:
        ...
        pipeline.xadd(global_key, MeterReadingSchema().dump(meter_reading), maxlen=self.GLOBAL_MAX_FEED_LENGTH)
        pipeline.xadd(feed_key, MeterReadingSchema().dump(meter_reading), maxlen=self.SITE_MAX_FEED_LENGTH)
